...
(function (algolia) {
  ...
  algolia.helpers = {
    ...
    GlformattedPriceWithComparison: function formattedPriceWithComparison () {
      var comparing = +this.compare_at_price && +this.compare_at_price > +this.price,
          discount_ratio = 1.0 - this.price_ratio,
          
          res = '<b style="display:none">' + 'formatPrice(this.price)'+ '</b>';
      if (comparing) {
        
        res += ' <span class="red ais-hit--price-striked" style="display:none"><span>' + formatPrice(this.compare_at_price) + '</span></span> ';
        res += ' <span class="red ais-hit--price-discount" style="font-weight:bold; ' + (Math.floor(discount_ratio * 10) * 100) + ';">' + 'Save '+ + Math.floor(discount_ratio * 100) + '%</span>';
      }

      return res;
    },
    PriceformattedPriceWithComparison: function formattedPriceWithComparison () {
      var comparing = +this.compare_at_price && +this.compare_at_price > +this.price,
          discount_ratio = 1.0 - this.price_ratio,
          res = '<b>' + formatPrice(this.price) + '</b>';
